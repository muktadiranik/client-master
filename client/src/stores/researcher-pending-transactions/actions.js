...
export const actions = {
  async [LOAD_TRANSACTIONS]({ commit, dispatch, state }, payload) {
    try {
      ...
      const response = await TransactionService.getPendingResearcherTransactionsList({
  ...
  async [SET_TRANSACTIONS_QUERY_TITLE]({ commit, dispatch }, payload) {
    commit(SET_TRANSACTIONS_QUERY_TITLE, payload)
    await dispatch(LOAD_TRANSACTIONS)
  },
}
